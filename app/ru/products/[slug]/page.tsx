...
export default async function RussianProductPage({ params }: Props) {
  const { slug } = await params;
  const decodedSlug = decodeURIComponent(slug);
  const product = await getProductBySlug(decodedSlug);
  if (!product) notFound();
  if (product.slug !== decodedSlug) permanentRedirect(`/ru/products/${product.slug}`);
  ...
  return <LocaleProductDetailPage locale="ru" product={product} related={related} sameBrand={sameBrand} />;
}
